...
function hazRollDice_install() {
	global $db;
	$db->write_query("CREATE TABLE mybb_haz_rolldice( pid int(10), uid int(10), type int(10), claim VARCHAR(100), result VARCHAR(200), reason VARCHAR(100))");
}
// Uninstall plugin
...
function hazRollDice_buildRollBar() {
	...
	while(null!=($result = $db->fetch_array($post_dices_query))){
		$text .= "<tr><td><b>".$result['claim']."</b> => </td>"
				."<td>".$result['result']." = </td>"
				."<td><b style=color:blue;>".$result['sum']."</b></td>"
				."<td>".preg_replace("/\"/", "\\\"", $result['reason'])."</td></tr>";
	}
	...
	eval("\$hazDice_bar .= \"".$templates->get("hzrd_bar")."\";");
}
	...
	if($post['pid']){
		...
		while(null!=($hazPost = $db->fetch_array($query))){
			...
			if($hazPost["claim"][0]=='s'){
				$dice_result = "<td colspan='3' style='color:white;background:black;text-align:center;'><strong>--[".$lang->hazRollDice_secret_dice."]--</strong></td>";
			}
			else{
				$dice_result = "<tr><td><strong>".$hazPost["claim"]." =></strong></td>"
								."<td>".$hazPost["result"]." = </td>"
							 	."<td><strong style='color:blue;'>".$hazPost["sum"]."</strong></td>";
			}
			$text .= $dice_result
		...
		if($count){
			...
			eval("\$post[hazDice_result] .= \"".$templates->get("hzrd_result")."\";");
		}
	}
}
...
function hazRollDice_handleResultRecord() {
	...
	if($_POST['action'] == "hz_rolldice") {
		$new_record = array(
			...
			"sum" 		=> $mybb->get_input("haz_dices_sum"),
